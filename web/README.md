...
- **同页多会话并发流**：流式状态按 `thread_id` 隔离（`streamingThreads` / `messagesByThread` / `pendingInterrupts`），会话 A 流式输出时可随时切换或新建会话 B 并发送消息，两路 SSE 互不干扰
- 切换离开进行中的会话后，其半成品回复保存在该线程的消息缓冲里，切回时继续展示增量
- 「停止」按钮只中止当前线程的流，不影响后台其他会话
- **停止即时生效**：点击停止时前端先调用 `POST /api/stop` 向后端发送显式取消信号，再 abort 连接。后端据此在 LLM 阻塞调用（含 SDK 内部 429/5xx 自动重试）期间也能立即中断生成并返回 `cancelled` 事件；同时取消该会话待处理的记忆沉淀（后台防抖 fact 抽取），彻底杜绝「点了停止后端还在重试」。
- 挂起中断（HITL `ask_human`）按线程独立记录，仅当切回对应会话时展示
