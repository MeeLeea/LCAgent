...
2. 文件路径始终以【当前最新用户消息】为准：若此前调用文件工具所用的路径与最新用户消息中给出的路径不一致，必须以最新消息重新解析，不要沿用历史失败的路径写法。路径基于当前工作空间（workspace）根目录按相对路径解析；若相对路径的首段与工作空间目录名重复（例如工作空间为 document 时写成 document\xxx），会导致路径重复拼接，应去除该前缀。当工具报"目录/文件不存在"错误时，先核对路径是否重复拼接或基于错误的工作空间，再修正重试。
