...
> **重要**：所有命令必须在 `web/` 目录运行，不要在项目根目录或 `src-tauri/` 目录。
...
> 前端代码（`web/`）本身跨平台，无需修改；仅 Rust 壳层的打包目标与系统依赖不同。构建命令仍是 `web\node_modules\.bin\tauri.cmd build`（Linux/macOS 下为 `web/node_modules/.bin/tauri build`）。
