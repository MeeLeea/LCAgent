...
从 agent/skill_ops.py 迁入(原样,含 manually_compact,不拆分)。
职责:
- 技能列表 / 加载到会话 state / 清空会话技能
- 手动触发上下文压缩(CLI compact 命令入口)

作为 AgentCore 多继承 Mixin,自身不初始化状态,依赖宿主类的实例属性:
skill_manager / agent_executor / _compaction_middleware / metrics / verbose / session。
运行时用 TYPE_CHECKING 引 AgentCore 避免循环导入。
"""
...
class SkillOps:
    ...
    async def aclear_skills(self, thread_id: str | None = None):
        ...
        注:只清 state 的 active_skills,不影响 TeamAgent.fixed_skills 类属性
