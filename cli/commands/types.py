...
class MemoryLike(Protocol):
    """长期记忆 + checkpointer 接口。

    会话管理（new/switch/list/delete/get_messages 等）已迁移至 SessionLike。
    本接口仅保留长期记忆操作和 checkpointer 配置。
    """

    thread_id: str

    def get_config(self, thread_id: str | None = None) -> dict[str, JsonValue]: ...
    def get_long_term(self, limit: int = 5) -> list[dict[str, str]]: ...
    def clear_long_term(self) -> None: ...
    def compress_memory(self, summarize_callback: Callable[[str, str], str]) -> dict[str, JsonValue]: ...
    async def aadd(self, role: str, content: str, metadata: dict[str, JsonValue] | None = None) -> None: ...
    async def aclear_long_term(self) -> None: ...
    async def acompress_memory(self, summarize_callback: Callable[[str, str], Awaitable[str]]) -> dict[str, JsonValue]: ...
