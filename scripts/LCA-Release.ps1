# LangChain Agent Launcher (Release Mode)
# Usage: .\LCA-Release.ps1 [-build|-run|-all] or .\LCA-Release.ps1 [build|run|all]
...
if ($Action -notin @("all", "build", "run")) {
    ...
    Write-Host "  .\LCA-Release.ps1              # all (default)"
    Write-Host "  .\LCA-Release.ps1 build        # build only"
    Write-Host "  .\LCA-Release.ps1 run          # run only"
    Write-Host "  .\LCA-Release.ps1 -build       # build only"
    Write-Host "  .\LCA-Release.ps1 -run         # run only"
    exit 1
}
...
function Build-Tauri-Release {
    Write-Step "Build Tauri Desktop (Release mode)"
    Write-Hint "Full optimization, 2-5 minutes"
    ...
    Write-OK "Tauri build completed (Release)"
    
    $exePath = Join-Path $TauriDir "target\release\langchainagent.exe"
    Write-Hint "Binary: $exePath"
}
...
function Start-Desktop-Release {
    Write-Step "Start Desktop App (Release)"
    $exePath = Join-Path $TauriDir "target\release\langchainagent.exe"
    
    if (-not (Test-Path $exePath)) {
        Write-Fail "Desktop app not found: $exePath"
        Write-Hint "Run: .\LCA-Release.ps1 build"
        exit 1
    }
    
    Start-Process -FilePath $exePath -WorkingDirectory $ProjectRoot
    Write-OK "Desktop app started (Release mode)"
}

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "  LangChain Agent Launcher (Release)" -ForegroundColor Cyan
Write-Host "  Mode: $Action" -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Cyan

switch ($Action) {
    "build" { Build-Frontend; Build-Tauri-Release }
    "run" { Start-Backend; Start-Desktop-Release }
    "all" { Build-Frontend; Build-Tauri-Release; Start-Backend; Start-Desktop-Release }
}
...
if ($Action -eq "build") {
    Write-Hint "Next: .\LCA-Release.ps1 run"
} elseif ($Action -eq "run") {
    Write-Hint "Rebuild: .\LCA-Release.ps1 build"
} else {
    Write-Hint "Using Release build (optimized for production)"
    Write-Hint "For development: .\LCA.ps1"
}
Write-Host ""
